...
function AuctionPage() {
    ...
    async function fetchProductData(grNo) {
        ...
        try {
            ...
            if (response.data == 'noProduct') alert('상품이 없습니다.');
            else {
    ...
    async function fetchExtendLevel(grNo) {
        ...
        try {
            ...
            if (response.data === '') alert('상품이 없습니다.');
            else {
    ...
    async function fetchUserPoint() {
        ...
        try {
            ...
            if (response.data.length > 0) setUserPoint(response.data[0].P_CURRENT);
        } catch (error) {
            console.error(error);
        }
    }
    ...
    async function getMaxLevelBidId() {
        ...
        try {
            ...
            if (response.data === '') alert('등록된 아이디가 없습니다.');
            else {
        ...
        try {
            if (product) {
                const response = await axios.get(
                    `${SERVER_URL.SERVER_URL()}/auction/biding?grNo=${
                        product.GR_NO
                    }&asPrice=${nextBid}&extendLevel=${level}`
                );
    ...
    async function endedAuction() {
        ...
        if (product.M_ID === undefined) product.M_ID = null;
    ...
    async function asBid(level) {
        ...
        try {
            if (product) {
                const response = await axios.get(
                    `${SERVER_URL.SERVER_URL()}/auction/asBiding?grNo=${
                        product.GR_NO
                    }&asPrice=${asPrice}&extendLevel=${level}`
                );
                if (response.data == 'fail') {
                    alert('입찰에 실패 했습니다.');
                    window.location.reload();
    ...
    const asBidBtnHandler = () => {
        ...
        if (checkAsBid(asPrice) && checkId()) {
            if (hour === 0 && minutes <= 5 && extendLevel > 6) {
                ...
                if (!confirm('단 한번만 입찰이 가능 합니다. 이 금액으로 입력 하시겠습니까?')) return;
            }
            sessionCheck(sessionId, navigate);
            setLoaingModalShow(true);
            setIsBidType(true);
            ...
            <div className="auction_page_wrap">
                <div className="auction_semi_info">
                    <div className="auction_back_img">
                        <button onClick={leftBtnClickHandler}>
                            <img src="/img/auction_page/arrow_left.png" />
                        </button>
                        ...
                        <button>
                            <img src="/img/auction_page/arrow_right.png" onClick={rightBtnClickHandler} />
                        </button>
                    </div>
                        ...
                        <div className="product">
                            {product && (
                                <>
                                    ...
                                    <div className="poduct_info_sub">
                                        <h3>시작가 &nbsp; | </h3>
                                        <h3>{product.GR_PRICE.toLocaleString('ko-KR')}원</h3>
                                    </div>
                                    <div className="poduct_info_sub">
                                        <h3>현재가 &nbsp; | </h3>
                                        <h3>{nowPrice.toLocaleString('ko-KR')}원</h3>
                                    </div>
                                    <div className="poduct_info_sub">
                                        <h3>판매자 &nbsp; | </h3>
                                        <h3>{product.M_ID}</h3>
                                    ...
                                    <h3>물건 소개</h3>
                                    <div className="product_info_txt">{product.GR_INFO}</div>
                                </>
                            )}
                        </div>
                ...
                <div className="auction_area">
                    <div className="auction_bird">
                        <img id="bid_bird" src="/img/auction_page/auction_back_bubble.png" alt="" />
                        <span className="bubble_text">
                            ...
                            {extendLevel === 7 && minutes <= 5 ? (
                                ...
                                <>
                                    {bidingLog.length === 0 ? (
                                        <>
                                            남은 경매 시간 {hour < 10 ? '0' + hour : hour}:
                                            {minutes < 10 ? '0' + minutes : minutes}:
                                            {seconds < 10 ? '0' + seconds : seconds}입니다.
                                        </>
                                    ) : (
                                        <>
                                            ...
                                            <span>
                                                {bidingLog.length > 0
                                                    ? bidingLog[bidingLog.length - 1].AC_POINT.toLocaleString('ko-KR')
                                                    : ''}
                                            </span>{' '}
                                            원에 상회 입찰 하였습니다.
                                            <br />
                                            <br />
                                            남은 시간은 {hour < 10 ? '0' + hour : hour}:
                                            {minutes < 10 ? '0' + minutes : minutes}:
                                            {seconds < 10 ? '0' + seconds : seconds + ' '}입니다.
                                        </>
                                    )}
                            ...
                            <div className="text_log">
                                {bidingLog.map((biding) => (
                                    <div key={biding.AC_NO}>
                                        [{biding.AC_REG_DATE}] {biding.M_ID}님 께서{' '}
                                        {biding.AC_POINT.toLocaleString('ko-KR')}원 에 상회 입찰 하였습니다.
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="auction_btn">
                            <div className="nomarlbidBtn_wrap">
                                <button onClick={normalBidBtnHandler} disabled={isAuctionEnd} id="normalbidBtn">
                                    입찰({nextBid.toLocaleString('ko-KR')}₩)
                                </button>
                            </div>
                            <br />
                            <div className="call_bid">
                                <div>
                                    <input
                                        ...
                                        onChange={(e) => asPriceChangeHandler(e)}
                                    />
                                </div>
                                <div>
                                    <button onClick={asBidBtnHandler} disabled={isAuctionEnd} id="hogaBidBtn">
                                        호가 입찰
                                    </button>
                                </div>
                            </div>
